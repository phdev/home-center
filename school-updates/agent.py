...
def publish_gmail_error(error, cfg):
    """Surface Gmail auth/connectivity failures as a Howie action."""
    message = str(error or "Gmail fetch failed").strip()
    auth_failure = "invalid_grant" in message or "expired or revoked" in message
    title = "Reconnect Gmail for school emails" if auth_failure else "Check school email sync"
    summary = (
        "Howie cannot read school emails because Google authorization is expired or revoked."
        if auth_failure
        else f"Howie could not read school emails: {message[:180]}"
    )
    item = {
        "id": "gmail-auth-invalid-grant" if auth_failure else "school-email-sync-error",
        "kind": "action",
        "title": title,
        "summary": summary,
        "dueDate": datetime.now(timezone.utc).isoformat(),
        "urgency": 1.0,
        "suggestedAction": "Reauthorize Gmail/Calendar for Howie, then rerun school updates.",
        "child": "Howie",
        "classifier": "system",
        "sourceEmailId": "gmail-auth" if auth_failure else "school-email-sync",
    }
    publish_updates([item], cfg)
...
def run_once(cfg, seen):
    ...
    if "error" in result and not result.get("emails"):
        ...
        publish_gmail_error(result["error"], cfg)
        return
