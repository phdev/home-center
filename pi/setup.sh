...
log "Publishing dashboard assets..."
DASHBOARD_LOCAL_DIR="$REPO_DIR/dashboard-local"
mkdir -p "$DASHBOARD_LOCAL_DIR/home-center"
rsync -a --delete "$REPO_DIR/dist/" "$DASHBOARD_LOCAL_DIR/home-center/"
...
# Kiosk watchdog (recovers Chromium "Aw, Snap" renderer crashes)
...
sudo sed -i "s|__REPO_DIR__|$REPO_DIR|g" /etc/systemd/system/morning-dashboard.service
sudo sed -i "s|__USER__|$USER|g" /etc/systemd/system/morning-dashboard.service
sudo cp "$SCRIPT_DIR/services/morning-dashboard.timer" /etc/systemd/system/
