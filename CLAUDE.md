...
3. **OpenClaw enhances, it does not decide.** Copy, summaries, ordering
   ...
   Household memory follows the same rule: it may inform Howie's conversation
   and suggestions, but cannot directly drive UI behavior. Any UI behavior must
   be modeled through derived state, docs, tests, and AgentCI.
...
Home Center also documents a gstack workflow for OpenClaw named agents Devon
and David, with Codex as Devon's repo-editing harness. This workflow augments
the rules above; it does not replace the gbrain contract or the raw data →
derived state → UI presentation boundary.
...
Before building anything else, split broad dirty state into small reviewable
commits. Use this workflow whenever `git status --short` shows multiple
concerns or generated artifacts mixed with implementation:

1. Start each workstream with `git status --short` and name the intended commit
   bucket before editing.
2. After each coherent concern, run the smallest relevant test or inspection and
   commit immediately.
3. Keep generated artifacts in their own commit, separate from implementation.
4. Regenerate `docs/status/devon-project-index.md` only after intentional
   commits are made, so it records a clean known point.
5. For design-to-implementation work, commit design memory/artifacts first,
   implementation second, and verification/index updates last. Include
   `docs/DESIGN_MEMORY.md` with design-memory/artifact commits when its body
   changes; skip or revert timestamp-only diffs.
6. If a quick fix appears while working on something else, either commit/stash
   the current bucket first or write it down and defer it.

Do not start new feature work on top of unrelated dirty files unless Peter
...
For TV dashboard or kiosk design requests, update the live Pencil file via MCP first, then update the implementation, build, deploy to the Pi, and verify the served bundle. A repo-only design/code update is not complete unless the Pi deploy is done or explicitly blocked.
...
1. Check `voice-service/logs/voice-reliability.jsonl` for recent
   `detector_text`, wake hits, command transcripts, parser results, and
   dispatches.
2. Check `voice-service/logs/voice-stderr.log` for the human-readable command
   transcript and `Dispatching: ...` lines.
...
Specific 2026-05-05 lesson: `Hey Homer, open calendar` was misheard by Vosk as
`hey i'm robyn calendar`. That looked like a calendar navigation failure, but
the real failure was wake recognition. The narrow alias is now covered in
...
- **Speech candidate cadence:** speech mode uses `SPEECH_CANDIDATE_COOLDOWN_SECONDS`, not the generic wake cooldown. Keep it short during 5-phrase validation so adjacent commands are not suppressed. Keep `WHISPER_NO_SPEECH_THRESHOLD` at the default `0.45` unless a targeted short-command trial proves otherwise; `0.95` recovered one 5-phrase run but caused too much passive Whisper churn and hallucinated non-command transcripts.
- **Speech candidate empty backoff:** `SPEECH_CANDIDATE_EMPTY_BACKOFF_SECONDS` backs off after any empty/non-command speech confirmation. Strong speech segments can still confirm during that window when they meet `SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_PEAK_RMS` and `SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_ACTIVE_CHUNKS`; this is meant to reduce passive Whisper churn, but set it to `0` if it blocks short follow-up commands during 5-phrase validation. Keep `SPEECH_CANDIDATE_MAX_EMPTY_BACKOFF_SECONDS` for long ambient-speech churn.
...
- **Current post-merge follow-up:** the 2026-05-03 post-merge speech dry-run initially reached 3/5 because speech mode inherited the generic 3s wake cooldown and dropped adjacent commands. With `SPEECH_CANDIDATE_COOLDOWN_SECONDS=0.5`, `SPEECH_CANDIDATE_MAX_EMPTY_BACKOFF_SECONDS=30.0`, and the default `WHISPER_NO_SPEECH_THRESHOLD=0.45`, the final 5-phrase dry-run passed 5/5 with median wake-to-action latency around 288 ms and p95 around 335 ms. After the confirmed-command `stop` and explicit-ask guards, `SPEECH_CANDIDATE_EMPTY_BACKOFF_SECONDS=8.0` with strong override peak `1800`/active chunks `8` also passed the 5-phrase dry-run at 5/5 with median wake-to-action latency around 290 ms. The matching 10-minute passive run produced 0 dispatches and 0 command candidates with 30 internal Whisper confirmations and 20 `empty_backoff` skips, down from 71 internal confirmations in the previous 10-minute passive baseline.
- **Current long passive finding:** the 2026-05-03 10-minute passive speech run produced one dry-run false command candidate from a Whisper hallucination, `Hey Homer, stop the perfect`. The intent parser now only accepts `stop`, `dismiss`, `cancel`, and timer-control variants when there are no arbitrary trailing words. The repeat passive run then exposed `Hey Homer, what's going on? That was very enthusiastic`, which reached the open-ended `ask` path. Confirmed-command mode now requires explicit `ask`, `tell me`, `explain`, or `describe` cues for ask dispatches. Re-run the longer passive check before any 20-command run.
...
- In confirmed-command mode, "Hey Homer, ask/tell me/explain/describe ..." routes the question to the existing worker `/api/ask-query`; bare "Hey Homer, what/who/where/when/why/how ..." questions are intentionally ignored there to reduce passive-TV false dispatches.
