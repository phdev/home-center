...
Install the recurring voice health check after the voice service is installed:

```bash
bash deploy/mac-mini/setup-voice-health.sh
tail -f voice-service/logs/voice-health.log
```

This loads `com.homecenter.voice-health`, which runs every 15 minutes. It
checks parser regressions for known wake variants, confirms
`com.homecenter.voice` is running, verifies the Pi `/api/navigate` endpoint,
and writes `voice-service/logs/voice-health-status.json`.
...
- `OPENAI_STT_MODE`: `off` by default; set to `fallback` to call OpenAI only
  when local `faster-whisper` does not parse a dispatchable command, or
  `diagnostic` to log cloud/local comparisons without changing dispatch
...
- `POST_ACTION_MUTE_SECONDS`: default `3.0`; production launchd uses `0.15`
  so back-to-back navigation commands are not held behind a long mute window
- `WAKE_COOLDOWN`: default `3.0`; production launchd uses `0.75` so a fresh
  "Hey Homer" can be accepted soon after a completed command
- `PRE_WAKE_SECONDS` / `COMMAND_MIN_SECONDS` / `POST_WAKE_SILENCE_SECONDS`:
  normal Vosk-path command capture timing. Production launchd uses
  `2.4` / `0.16` / `0.08` for snappier simple commands while preserving the
  `1.6s` max for longer timer phrases.
- `FOLLOWUP_COMMAND_WAIT_SECONDS` / `FOLLOWUP_COMMAND_MAX_SECONDS`: after a
  bare wake phrase with no parsed command, keep listening for a brief paused
  follow-up command without requiring the user to say "Hey Homer" again.
  Production launchd uses `2.0` / `3.0`.
...
both agents. After that, ordinary messages to the design bot receive a short
David design/frontend response. Explicit memory feedback sent with
`/feedback ...`, `/remember ...`, `/memory ...`, `feedback: ...`,
`remember: ...`, or `memory: ...` is picked up within 5 minutes, parsed via the
feedback prompt, merged into `design_memory/`, and acked.
