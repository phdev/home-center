...
- **Deterministic derived state drives the UI.** Visibility is a flag,
  not an LLM result.
- **Design output is source material.** If you suggest card priority,
  ordering, suppression, fallback copy, or new flags, phrase it as a proposal
  for `/plan-eng-review`, not as approved deterministic behavior.
...
```json
{
  ...
  "test_candidates": ["2-3 strong concrete variants or hypotheses to test next, each one sentence"],
  "memory_alignment": {
...
- **Strong test candidates.** `test_candidates` are not alternate designs;
  they are the 2-3 strongest implementation hypotheses to try after seeing
  the mockup image.
...
- **No unilateral card logic.** Do not say "implement this deterministic
  priority rule" or equivalent. Say what the mockup is testing and what
  derived-state/card-contract question Devon must review before implementation.
