...
from health_check import HealthError, check_reliability_log
...
def test_health_check_rejects_fresh_but_dead_mic_log(tmp_path, monkeypatch):
    monkeypatch.setattr("health_check.time.time", lambda: 1000.0)
    path = tmp_path / "voice-reliability.jsonl"
    path.write_text(
        "\n".join(
            [
                '{"event":"detector_text","ts":990,"rms":1,"noise":1}',
                '{"event":"detector_text","ts":995,"rms":1,"noise":1}',
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    try:
        ...
        assert "abnormally quiet" in str(exc)
    else:
        raise AssertionError("expected quiet reliability log to fail health check")


def test_health_check_accepts_recent_mic_activity(tmp_path, monkeypatch):
    monkeypatch.setattr("health_check.time.time", lambda: 1000.0)
    path = tmp_path / "voice-reliability.jsonl"
    path.write_text(
        '{"event":"detector_text","ts":995,"rms":260,"noise":42}\n',
        encoding="utf-8",
    )

    result = check_reliability_log(
        path,
        max_age_seconds=3600,
        min_recent_rms=50,
        min_recent_noise=20,
    ...
    assert result["recentEvents"] == 1
    assert result["maxRecentRms"] == 260
