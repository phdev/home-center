...
from voice_service import (
    ...
    build_wake_detector,
    capture_followup_command,
    command_from_followup_transcript,
...
def test_capture_followup_command_waits_for_speech_after_pause():
    quiet = np.full(1280, 20, dtype=np.int16)
    speech = np.full(1280, 900, dtype=np.int16)
    mic = FakeMic([quiet, quiet, quiet, speech, speech, speech, quiet, quiet, quiet])
    rolling = FakeRolling()

    audio, waited_seconds, speech_seconds = capture_followup_command(
        mic,
        rolling,
        noise_floor=80,
        wait_seconds=1.0,
    ...
    assert waited_seconds == 0.32
    assert speech_seconds >= 0.4
    assert len(audio) >= 5 * 1280
    assert len(rolling.chunks) >= 6


def test_capture_followup_command_times_out_without_speech():
    quiet = np.full(1280, 20, dtype=np.int16)
    mic = FakeMic([quiet] * 8)
    rolling = FakeRolling()
...
def test_voice_reliability_logger_writes_jsonl_and_activity_summary(tmp_path, monkeypatch):
    times = iter([10.0, 10.1, 10.2, 15.3, 15.4])
    monkeypatch.setattr("voice_service.time.time", lambda: next(times))
    path = tmp_path / "voice-reliability.jsonl"
    logger = VoiceReliabilityLogger(path, activity_interval_seconds=5.0, activity_min_rms=300)

    logger.event("service_ready", wakeEngine="vosk")
    logger.record_activity(80, noise=50, gate=180, wake_engine="vosk")
    logger.record_activity(420, noise=55, gate=180, wake_engine="vosk", detector_source="partial", detector_text="hey")

    lines = path.read_text().splitlines()
    assert len(lines) == 2
    assert '"event":"service_ready"' in lines[0]
    assert '"event":"speech_activity"' in lines[1]
    assert '"maxRms":420' in lines[1]
...
def test_openai_stt_attempt_policy_keeps_local_as_default():
    assert should_attempt_openai_stt("off", local_dispatchable=False) == (False, "off")
    assert should_attempt_openai_stt("fallback", local_dispatchable=True) == (False, "local_dispatchable")
    assert should_attempt_openai_stt("fallback", local_dispatchable=False) == (True, "fallback")
    assert should_attempt_openai_stt("diagnostic", local_dispatchable=True) == (True, "diagnostic")


def test_openai_stt_fallback_can_recover_failed_local_parse(monkeypatch):
    monkeypatch.setattr("voice_service.transcribe_openai", lambda *_args, **_kwargs: "Hey Homer, stop")

    diagnostic = run_openai_stt_if_needed(
        np.ones(1600, dtype=np.int16),
        mode="fallback",
        local_dispatchable=False,
        api_key="test-key",
        model_name="gpt-4o-transcribe",
        timeout_seconds=1.0,
        parse_transcript=lambda text: (*command_from_transcript(text), []),
    )

    assert diagnostic.attempted
    assert diagnostic.reason == "fallback"
    assert diagnostic.transcript == "Hey Homer, stop"
    assert diagnostic.body == "stop"
    assert diagnostic.command == {"action": "stop"}
    assert diagnostic.error == ""


def test_openai_stt_fallback_skips_when_local_parse_succeeds(monkeypatch):
    calls = []
    monkeypatch.setattr("voice_service.transcribe_openai", lambda *_args, **_kwargs: calls.append("called"))

    diagnostic = run_openai_stt_if_needed(
        np.ones(1600, dtype=np.int16),
        mode="fallback",
        local_dispatchable=True,
        api_key="test-key",
        model_name="gpt-4o-transcribe",
        timeout_seconds=1.0,
        parse_transcript=lambda text: (*command_from_transcript(text), []),
    ...
    assert not diagnostic.attempted
    assert diagnostic.reason == "local_dispatchable"
    assert calls == []
...
def test_command_from_transcript_ignores_bare_wake():
    ...
    assert command_from_transcript("Hey Howie.", fallback_text="hey howie") == ("", {"action": "none"})
...
def test_command_from_transcript_routes_howie_wake_to_telegram_intent():
    assert command_from_transcript("Hey Howie, ask Devon if the second Pi is healthy") == (
        "howie ask Devon if the second Pi is healthy",
        {"action": "howie_message", "message": "ask Devon if the second Pi is healthy"},
    )


def test_howie_followup_routes_bare_transcript_to_telegram_intent():
    assert command_from_followup_transcript("find guess", howie_context=True) == (
        "howie find guess",
        {"action": "howie_message", "message": "find guess"},
    )


def test_howie_openai_followup_parser_treats_plain_text_as_howie_message():
    assert parse_howie_openai_transcript("find Jess Ivy", wake_text="hey howie", followup_context=True) == (
        "howie find Jess Ivy",
        {"action": "howie_message", "message": "find Jess Ivy"},
        [],
    )


def test_howie_fallback_routes_non_command_wake_speech():
    assert howie_fallback_from_transcript(
        "Hey Homer, commands are not being detected",
        wake_text="hey homer",
    ) == (
        "howie commands are not being detected",
        {"action": "howie_message", "message": "commands are not being detected"},
    )


def test_howie_fallback_does_not_steal_home_commands():
    assert howie_fallback_from_transcript("Hey Homer, open calendar", wake_text="hey homer") == (
        "",
        {"action": "none"},
    )


def test_command_from_transcript_keeps_negative_knowledge_feedback_on_homer():
    assert command_from_transcript("Hey Homer, bad answer") == (
        "bad answer",
        {"action": "knowledge_feedback", "feedback_type": "knowledge"},
    )
    assert command_from_transcript("Hey Homer, bad image") == (
        "bad image",
        {"action": "knowledge_feedback", "feedback_type": "image"},
    )
...
def test_dispatchable_commands_from_transcript_splits_howie_messages():
    assert dispatchable_commands_from_transcript("Hey Howie, send this to OpenClaw.") == [
        {"body": "howie send this to OpenClaw", "command": {"action": "howie_message", "message": "send this to OpenClaw"}},
    ]
...
def test_confirmed_command_accepts_wake_knowledge_question():
    assert confirmed_command_from_transcript(
        ...
        "how big is the sun",
        {"action": "ask", "query": "how big is the sun"},
        [{"body": "how big is the sun", "command": {"action": "ask", "query": "how big is the sun"}}],
    )


def test_dispatch_ask_uses_openclaw_router_bridge(monkeypatch):
    calls = []
    ...
    dispatcher.dispatch({"action": "ask", "query": "why is the sky blue"})

    assert calls == [
        {
            "url": "http://127.0.0.1:3100/ask-query",
...
def test_command_from_transcript_hybrid_fallback_completes_split_turn_on():
    assert hybrid_command_body_candidates("on", fallback_text="Hey Homer, turn") == [
        "on",
        "turn",
        "turn on",
    ]
    assert command_from_transcript("on", fallback_text="Hey Homer, turn") == (
        "turn on",
        {"action": "turn_on"},
    )


def test_confirmed_command_hybrid_fallback_completes_split_go_home():
    assert confirmed_command_from_transcript(
        "Hey Homer, home",
        fallback_text="Hey Homer, go",
        require_wake_phrase=True,
    ) == (
        "go home",
