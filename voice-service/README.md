...
```bash
OPENAI_API_KEY=... \
OPENAI_STT_MODE=fallback \
OPENAI_STT_MODEL=gpt-4o-transcribe \
python voice_service.py --dry-run --debug
```

`OPENAI_STT_MODE=fallback` only calls OpenAI when the local transcript does not
parse into a dispatchable command; a recovered cloud parse can then dispatch.
`OPENAI_STT_MODE=diagnostic` calls OpenAI alongside local STT and records the
comparison in the reliability log, but local parsing still controls dispatch.
Keep `OPENAI_STT_MODE=off` for fully local operation.
