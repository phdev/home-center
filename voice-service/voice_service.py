...
PRE_WAKE_SECONDS = float(os.environ.get("PRE_WAKE_SECONDS", "2.4"))
COMMAND_MAX_SECONDS = float(os.environ.get("COMMAND_MAX_SECONDS", "1.6"))
COMMAND_MIN_SECONDS = float(os.environ.get("COMMAND_MIN_SECONDS", "0.16"))
POST_WAKE_SILENCE_SECONDS = float(os.environ.get("POST_WAKE_SILENCE_SECONDS", "0.08"))
FOLLOWUP_COMMAND_WAIT_SECONDS = float(os.environ.get("FOLLOWUP_COMMAND_WAIT_SECONDS", "2.0"))
FOLLOWUP_COMMAND_MAX_SECONDS = float(os.environ.get("FOLLOWUP_COMMAND_MAX_SECONDS", "3.0"))
FOLLOWUP_COMMAND_PRE_ROLL_SECONDS = float(os.environ.get("FOLLOWUP_COMMAND_PRE_ROLL_SECONDS", "0.24"))
...
SPEECH_CANDIDATE_MAX_EMPTY_BACKOFF_SECONDS = float(
    os.environ.get("SPEECH_CANDIDATE_MAX_EMPTY_BACKOFF_SECONDS", "30.0")
)
SPEECH_CANDIDATE_EMPTY_BACKOFF_SECONDS = float(os.environ.get("SPEECH_CANDIDATE_EMPTY_BACKOFF_SECONDS", "8.0"))
SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_PEAK_RMS = float(
    os.environ.get("SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_PEAK_RMS", "1800")
)
SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_ACTIVE_CHUNKS = int(
    os.environ.get("SPEECH_CANDIDATE_EMPTY_BACKOFF_STRONG_MIN_ACTIVE_CHUNKS", "8")
)
...
DEFAULT_RELIABILITY_LOG = Path(__file__).resolve().parent / "logs/voice-reliability.jsonl"
...
def normalize_openai_stt_mode(value: str) -> str:
...
def write_temp_wav(audio: np.ndarray) -> Path:
    fd, output_name = tempfile.mkstemp(prefix="home-center-stt-", suffix=".wav")
    os.close(fd)
...
def transcribe_openai(
    ...
    model_name: str,
    api_key: str,
    timeout_seconds: float = OPENAI_STT_TIMEOUT_SECONDS,
) -> str:
    if len(audio) == 0:
        return ""
    ...
    try:
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"OpenAI STT returned {response.status_code}: {response.text[:200]}")
        return response.text.strip()
    finally:
        wav_path.unlink(missing_ok=True)


def should_attempt_openai_stt(mode: str, local_dispatchable: bool) -> tuple[bool, str]:
    if mode == "off":
        return False, "off"
    if mode == "diagnostic":
        return True, "diagnostic"
    if not local_dispatchable:
        return True, "fallback"
    return False, "local_dispatchable"


def run_openai_stt_if_needed(
    audio: np.ndarray,
    mode: str,
    local_dispatchable: bool,
    api_key: str,
    model_name: str,
    timeout_seconds: float,
    parse_transcript: Callable[[str], tuple[str, dict, list[dict]]],
) -> OpenAISttDiagnostic:
    attempted, reason = should_attempt_openai_stt(mode, local_dispatchable)
    diagnostic = OpenAISttDiagnostic(enabled=mode != "off", attempted=attempted, reason=reason)
    if not attempted:
        return diagnostic
    if not api_key:
        diagnostic.error = "missing_api_key"
        return diagnostic

    started = time.time()
    try:
        text = transcribe_openai(audio, model_name, api_key, timeout_seconds)
    except Exception as exc:
        diagnostic.latency_ms = round((time.time() - started) * 1000)
        diagnostic.error = str(exc)
        return diagnostic

    body, command, candidates = parse_transcript(text)
...
class VoiceReliabilityLogger:
    """Append structured, non-audio diagnostics for voice misses."""

    def __init__(
        self,
        path: Path | None,
        enabled: bool = True,
        activity_interval_seconds: float = 5.0,
        activity_min_rms: float = 350.0,
    ):
        self.path = path
        self.enabled = enabled and path is not None
        self.activity_interval_seconds = activity_interval_seconds
        self.activity_min_rms = activity_min_rms
        self.activity = AudioActivityHeartbeat(activity_interval_seconds)
        self._lock = threading.Lock()
        if self.enabled:
    ...
    def record_activity(
        ...
        if not self.enabled:
            return
        self.activity.record(chunk_rms)
        now = time.time()
        if not self.activity.should_log(now):
            return
        snapshot = self.activity.snapshot_and_reset(now)
        if snapshot["max_rms"] < self.activity_min_rms:
            return
        self.event(
            "speech_activity",
            wakeEngine=wake_engine,
            detectorSource=detector_source,
            detectorText=detector_text,
            windowMs=round(snapshot["window"] * 1000),
            chunks=snapshot["chunks"],
            maxRms=round(snapshot["max_rms"]),
            avgRms=round(snapshot["avg_rms"]),
            noise=round(noise),
            gate=round(gate),
        )
...
class Dispatcher:
    def __init__(
        ...
        dry_run: bool,
        design_feedback_script: Path | None = None,
        openclaw_bridge_url: str | None = None,
        openclaw_chat_id: str | None = None,
    ):
        self.pi_base = pi_base.rstrip("/")
        self.worker_url = worker_url.rstrip("/") if worker_url else None
        self.worker_token = worker_token
        self.dry_run = dry_run
        self.design_feedback_script = design_feedback_script
        self.openclaw_bridge_url = openclaw_bridge_url.rstrip("/") if openclaw_bridge_url else None
        self.openclaw_chat_id = (openclaw_chat_id or "").strip()

    def _pi_post(self, path: str, payload: dict | None = None, timeout: float = 4.0) -> bool:
        if self.dry_run:
    ...
    def _openclaw_post(self, path: str, payload: dict | None = None, timeout: float = 8) -> bool:
        if not self.openclaw_bridge_url:
            ...
            return False
        if self.dry_run:
            log.info("[DRY RUN] OpenClaw POST %s %s", path, payload or {})
            return True
        try:
            r = requests.post(f"{self.openclaw_bridge_url}{path}", json=payload or {}, timeout=timeout)
            if not r.ok:
                log.warning("OpenClaw POST %s -> %d %s", path, r.status_code, r.text[:200])
                return False
            return True
        except requests.RequestException as e:
    ...
    def wake_ack(self, text: str, wake_score: float, chime: bool = True, **extra) -> None:
        payload = {
            "text": text,
            "is_wake": True,
            "ts": time.time(),
            "stage": "listening",
            "wakeScore": wake_score,
            "duration": 5.0,
            "chime": chime,
            **extra,
        }
        start = time.time()
        ok = self._pi_post("/api/wake-ack", payload, timeout=0.5)
        log.info("Wake ack request completed in %.0fms", (time.time() - start) * 1000)
        if not ok:
            log.info("Wake ack unavailable; falling back to transcription + chime endpoints.")
            self.transcription(text, True, wakeScore=wake_score, stage="listening", **extra)
            if chime:
                self.chime(wake_score)

    def wake_ack_async(self, text: str, wake_score: float, chime: bool = True, **extra) -> None:
        threading.Thread(
            target=self.wake_ack,
            args=(text, wake_score, chime),
            kwargs=extra,
            daemon=True,
        ).start()
    ...
    def transcription_async(self, text: str, is_wake: bool, **extra) -> None:
        threading.Thread(
            target=self.transcription,
            args=(text, is_wake),
            kwargs=extra,
            daemon=True,
        ).start()
    ...
    def dispatch(self, command: dict) -> None:
        ...
        if action == "set_timer":
            ...
            self._pi_post("/api/design-system", {"version": command.get("version", "v1")})
        elif action == "turn_off":
            self._pi_post("/api/tv/off")
        elif action == "turn_on":
            self._pi_post("/api/tv/on")
        elif action == "ask":
            self._openclaw_post("/ask-query", {"query": command.get("query", "")}, timeout=45)
        elif action == "design_feedback":
    ...
    def record_design_feedback(self, command: dict) -> None:
        if not self.design_feedback_script:
            log.warning("Design feedback script not configured; dropping %s", command)
            return
        args = [
            sys.executable,
            str(self.design_feedback_script),
            "--sentiment",
        ...
        if self.dry_run:
            log.info("[DRY RUN] Design feedback %s", args)
            return
        try:
            result = subprocess.run(args, capture_output=True, text=True, timeout=20)
        except (OSError, subprocess.TimeoutExpired) as exc:
            log.warning("Design feedback failed: %s", exc)
            return
        if result.returncode != 0:
            detail = (result.stderr or result.stdout).strip()[:400]
            log.warning("Design feedback exited %d: %s", result.returncode, detail)
            return
        log.info("Design feedback recorded: %s", result.stdout.strip()[:400])

    def send_howie_message(self, command: dict) -> None:
        message = str(command.get("message", "")).strip()
        if not self.openclaw_chat_id:
            log.warning("OpenClaw chat ID is not configured; dropping Howie message.")
            return
        if not message:
            log.warning("OpenClaw Howie message is empty; dropping command.")
            return
        sent = self._openclaw_post("/voice-inbound", {"chatId": self.openclaw_chat_id, "message": message})
        if sent:
            log.info("Queued Howie voice message for OpenClaw chat %s: %r", self.openclaw_chat_id, message[:160])
            self.transcription_async(
                "Sent to Howie",
                False,
                stage="sent",
                command=message,
...
def capture_followup_command(
    mic: MicStream,
    rolling: RollingAudio,
    noise_floor: float,
    wait_seconds: float = FOLLOWUP_COMMAND_WAIT_SECONDS,
    max_speech_seconds: float = FOLLOWUP_COMMAND_MAX_SECONDS,
    pre_roll_seconds: float = FOLLOWUP_COMMAND_PRE_ROLL_SECONDS,
    min_speech_seconds: float = FOLLOWUP_COMMAND_MIN_SECONDS,
    silence_seconds: float = FOLLOWUP_COMMAND_SILENCE_SECONDS,
) -> tuple[np.ndarray, float, float]:
    """Wait briefly after a bare wake phrase, then capture one speech segment."""
    speech_gate = max(220.0, noise_floor * 1.55)
    silence_gate = max(200.0, noise_floor * 1.45)
    wait_samples = int(wait_seconds * SAMPLE_RATE)
    max_speech_samples = int(max_speech_seconds * SAMPLE_RATE)
    pre_roll_chunks = max(1, int(pre_roll_seconds * SAMPLE_RATE / CHUNK_SIZE))
    quiet_chunks: deque[np.ndarray] = deque(maxlen=pre_roll_chunks)

    waited_samples = 0
    while waited_samples < wait_samples:
        chunk = mic.read_chunk()
        rolling.append(chunk)
        waited_samples += len(chunk)
        chunk_rms = rms(chunk)
        if chunk_rms >= speech_gate:
            chunks = list(quiet_chunks)
            chunks.append(chunk)
            speech_samples = len(chunk)
            silence_started_at: int | None = None
            while speech_samples < max_speech_samples:
                next_chunk = mic.read_chunk()
                rolling.append(next_chunk)
                chunks.append(next_chunk)
                speech_samples += len(next_chunk)
                next_rms = rms(next_chunk)
                elapsed = speech_samples / SAMPLE_RATE
                if elapsed >= min_speech_seconds and next_rms < silence_gate:
                    if silence_started_at is None:
                        silence_started_at = speech_samples
                    elif (speech_samples - silence_started_at) / SAMPLE_RATE >= silence_seconds:
                        break
                else:
                    silence_started_at = None
            audio = np.concatenate(chunks) if chunks else np.array([], dtype=np.int16)
            return audio, waited_samples / SAMPLE_RATE, speech_samples / SAMPLE_RATE
        quiet_chunks.append(chunk)

    return np.array([], dtype=np.int16), waited_samples / SAMPLE_RATE, 0.0


def _command_body_after_wake(text: str, match: re.Match[str]) -> str:
    body = text[match.end():].strip(" ,.:;!?-")
    if HOWIE_WAKE_PHRASE_RE.fullmatch(match.group(0)):
        return f"howie {body}".strip() if body else ""
    return body
...
def command_body_from_transcript(text: str, prefer_last_wake: bool = False, wake_re=WAKE_PHRASE_RE) -> str:
    ...
    if matches:
        ...
        return _command_body_after_wake(text, match)
    return text.strip(" ,.:;!?-")


def _dedupe_command_bodies(bodies: list[str]) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
    for body in bodies:
...
def hybrid_command_body_candidates(
    text: str,
    fallback_text: str = "",
    wake_re=WAKE_PHRASE_RE,
) -> list[str]:
    """Return transcript, detector fallback, and combined command-body candidates."""
    text_body = command_body_from_transcript(text, prefer_last_wake=True, wake_re=wake_re)
    fallback_body = (
        command_body_from_transcript(fallback_text, prefer_last_wake=True, wake_re=wake_re)
        if fallback_text
        else ""
    )
    ...
    if fallback_body and text_body and fallback_body.lower() != text_body.lower():
        bodies.append(f"{fallback_body} {text_body}")
    if not wake_re.search(text) and text:
        bodies.append(text.strip(" ,.:;!?-"))

    return _dedupe_command_bodies(bodies)
...
def command_from_followup_transcript(text: str, howie_context: bool = False) -> tuple[str, dict]:
    """Parse a follow-up utterance after a bare wake phrase."""
    followup = text.strip(" ,.:;!?-")
    if howie_context and followup and not HOWIE_WAKE_PHRASE_RE.search(followup) and not followup.lower().startswith("howie "):
        followup = f"howie {followup}"
    return command_from_transcript(followup)


def parse_howie_openai_transcript(text: str, wake_text: str, followup_context: bool) -> tuple[str, dict, list[dict]]:
    """Parse higher-quality cloud STT for Howie without trusting Vosk text as the message."""
    if followup_context:
        body, command = command_from_followup_transcript(text, howie_context=True)
    else:
        body, command = command_from_transcript(text, fallback_text=wake_text)
    return body, command, []


def howie_fallback_from_transcript(text: str, wake_text: str = "") -> tuple[str, dict]:
    """Treat non-command speech after a wake as a Howie message."""
    body = command_body_from_transcript(text, prefer_last_wake=True)
    if not body and wake_text:
        body = command_body_from_transcript(f"{wake_text} {text}", prefer_last_wake=True)
    body = body.strip(" ,.:;!?-")
    if len(body.split()) < 2:
        return "", {"action": "none"}
    if re.search(
        r"\b(open|show|go|back|home|dashboard|calendar|weather|photos?|pictures?|gallery|"
        r"turn|set\s+(?:a\s+)?timer|timer|stop|dismiss|cancel|quiet|shut\s+up)\b",
        body,
        re.IGNORECASE,
    ):
        return "", {"action": "none"}
    return f"howie {body}", {"action": "howie_message", "message": body}
...
def dispatchable_commands_from_transcript(
    ...
    def add_candidate(body: str) -> None:
        command = parse_command(
            body,
            allow_bare_ask=allow_bare_ask,
            allow_wake_knowledge=require_wake_phrase and not allow_bare_ask,
        )
        if is_dispatchable_command(command):
    ...
    if matches:
        ...
        for index, match in enumerate(matches):
            ...
            if HOWIE_WAKE_PHRASE_RE.fullmatch(match.group(0)):
                body = f"howie {body}".strip()
            add_candidate(body)
    else:
        if require_wake_phrase:
            return []
        for body in hybrid_command_body_candidates(text, fallback_text=fallback_text, wake_re=wake_re):
            add_candidate(body)
...
def confirmed_command_from_transcript(
    ...
    text_has_wake = bool(wake_re.search(text))
    if require_wake_phrase and not text_has_wake:
...
def main() -> None:
    ...
    parser.add_argument("--openclaw-bridge-url", default=os.environ.get("OPENCLAW_BRIDGE_URL", "http://127.0.0.1:3100"))
    parser.add_argument("--openclaw-chat-id", default=os.environ.get("OPENCLAW_CHAT_ID") or os.environ.get("TELEGRAM_CHAT_ID"))
    parser.add_argument(
        "--vosk-model-dir",
    ...
    parser.add_argument(
        "--openai-stt-mode",
        choices=["off", "fallback", "diagnostic"],
        default=OPENAI_STT_MODE,
        help="OpenAI STT use: off, fallback after local parse misses, or diagnostic comparison logging.",
    )
    parser.add_argument(
        "--openai-stt-model",
        default=OPENAI_STT_MODEL,
        help="OpenAI transcription model for fallback/diagnostic runs.",
    )
    parser.add_argument(
        "--openai-stt-timeout-seconds",
        type=float,
        default=OPENAI_STT_TIMEOUT_SECONDS,
    ...
    parser.add_argument(
        ...
        help="Force OpenAI transcription for Howie dictation while keeping normal commands local.",
    )
    parser.add_argument(
        "--reliability-log",
        default=os.environ.get("VOICE_RELIABILITY_LOG", str(DEFAULT_RELIABILITY_LOG)),
        help="JSONL diagnostics path for wake/command reliability. Set empty string to disable.",
    )
    parser.add_argument(
        "--reliability-activity-interval-seconds",
        type=float,
        default=float(os.environ.get("VOICE_RELIABILITY_ACTIVITY_INTERVAL_SECONDS", "5.0")),
        help="Seconds per speech-activity summary window in the reliability log.",
    )
    parser.add_argument(
        "--reliability-activity-min-rms",
        type=float,
        default=float(os.environ.get("VOICE_RELIABILITY_ACTIVITY_MIN_RMS", "350")),
        help="Minimum window peak RMS for speech-activity reliability events.",
    )
    parser.add_argument(
        "--design-feedback-script",
        default=os.environ.get("DESIGN_FEEDBACK_SCRIPT", str(repo_root / "scripts/record_design_feedback.py")),
        help="Local script used for 'I like/dislike this design' voice feedback. Set empty string to disable.",
    )
    ...
    if args.openai_stt_mode != "off" and not openai_api_key:
        log.warning("OpenAI STT %s requested but OPENAI_API_KEY is not set; cloud STT disabled.", args.openai_stt_mode)
        args.openai_stt_mode = "off"
    detector = build_wake_detector(args)
    design_feedback_script = Path(args.design_feedback_script) if args.design_feedback_script else None
    dispatcher = Dispatcher(
        args.pi_base,
        args.worker_url,
        args.worker_token,
        args.dry_run,
        design_feedback_script=design_feedback_script,
        openclaw_bridge_url=args.openclaw_bridge_url,
        openclaw_chat_id=args.openclaw_chat_id,
    )
    reliability_log_path = Path(args.reliability_log) if args.reliability_log else None
    reliability = VoiceReliabilityLogger(
        reliability_log_path,
        enabled=bool(args.reliability_log),
        activity_interval_seconds=args.reliability_activity_interval_seconds,
        activity_min_rms=args.reliability_activity_min_rms,
    )
    ...
    log.info(
        "Ready: wake=%s STT=%s whisperNoSpeech=%.2f preWake=%.1fs postWakeMax=%.1fs "
        "commandMin=%.2fs postSilence=%.2fs recentRms=%.1fs "
        "followupWait=%.1fs followupMax=%.1fs "
        ...
        FOLLOWUP_COMMAND_WAIT_SECONDS,
        FOLLOWUP_COMMAND_MAX_SECONDS,
    ...
    reliability.event(
        ...
        confirmCommand=args.wake_confirm_command,
        preWakeSeconds=args.pre_wake_seconds,
        postWakeMaxSeconds=COMMAND_MAX_SECONDS,
        reliabilityActivityMinRms=args.reliability_activity_min_rms,
        reliabilityActivityIntervalSeconds=args.reliability_activity_interval_seconds,
    ...
    while True:
        ...
        reliability.record_activity(
            chunk_rms,
            noise,
            active_speech_gate(noise, args.speech_candidate_min_active_rms, args.speech_candidate_active_rms_multiplier),
            args.wake_engine,
        )
        ...
        if args.wake_engine == "openwakeword":
            ...
            if not passes_recent_speech_gate(
                ...
                reliability.event(
                    ...
                    wakeEngine=args.wake_engine,
                    source=wake_source,
                    reason="recent_speech_gate",
                    text=wake_text,
                    rms=round(chunk_rms),
                    noise=round(noise),
                    recentPeak=round(stats["peak"]),
                    activeChunks=stats["active_chunks"],
                    gate=round(stats["gate"]),
                )
                continue
            ...
            if args.wake_confirm_command:
                ...
                if not segment_debounce.can_confirm(now):
                    ...
                    reliability.event(
                        ...
                        activeChunks=stats["active_chunks"],
                        gate=round(stats["gate"]),
                        cooldownRemaining=round(max(segment_debounce.cooldown_until - now, 0.0), 3),
                    )
                    continue
        ...
        if should_skip_speech_candidate_confirmation(
            ...
            reliability.event(
                ...
                rms=round(chunk_rms),
                noise=round(noise),
                cooldownRemaining=round(max(cooldown_until - wake_at, 0.0), 3),
            )
            continue
        ...
        if args.wake_confirm_command:
            ...
            stt_source = "local"
            openai_diag = run_openai_stt_if_needed(
                command_audio,
                args.openai_stt_mode,
                bool(dispatch_items),
                openai_api_key,
                args.openai_stt_model,
                args.openai_stt_timeout_seconds,
            ...
            openai_dispatch_items = confirmed_dispatches(
                openai_diag.body,
                openai_diag.command or {"action": "none"},
                openai_diag.candidates or [],
                dispatch_all=args.confirm_multi_command_dispatch,
            )
            if args.openai_stt_mode == "fallback" and not dispatch_items and openai_dispatch_items:
                log.info(
                    "OpenAI STT fallback recovered command transcript=%r body=%r command=%s latency=%sms",
                    openai_diag.transcript,
                    openai_diag.body,
                    openai_diag.command,
                    openai_diag.latency_ms,
                )
                text = openai_diag.transcript
                body = openai_diag.body
                command = openai_diag.command or {"action": "none"}
                command_candidates = openai_diag.candidates or []
                dispatch_items = openai_dispatch_items
                stt_source = "openai_fallback"
            elif openai_diag.attempted:
                log.info(
                    "OpenAI STT %s transcript=%r body=%r command=%s dispatchable=%s latency=%sms error=%r",
                    openai_diag.reason,
                    openai_diag.transcript,
                    openai_diag.body,
                    openai_diag.command,
                    bool(openai_dispatch_items),
                    openai_diag.latency_ms,
                    openai_diag.error,
                )
            stt_done = time.time()
            reliability.event(
                ...
                openaiStt={
                    "enabled": openai_diag.enabled,
                    "attempted": openai_diag.attempted,
                    "reason": openai_diag.reason,
                    "transcript": openai_diag.transcript,
                    "body": openai_diag.body,
                    "command": openai_diag.command,
                    "dispatchable": bool(openai_dispatch_items),
                    "latencyMs": openai_diag.latency_ms,
                    "error": openai_diag.error,
                },
                captureMs=round(len(command_audio) / SAMPLE_RATE * 1000),
                preMs=round(pre_log_samples / SAMPLE_RATE * 1000),
                postMs=round(post_seconds * 1000),
                totalMs=round((stt_done - wake_at) * 1000),
            )
            log.info(
            ...
            if dispatch_items:
                ...
                for index, item in enumerate(dispatch_items, start=1):
                    ...
                    dispatcher.dispatch_async(item_command)
                    dispatcher.transcription_async(
                        final_text,
                        True,
                        wakeScore=1.0,
                        stage="command",
        ...
        if is_dispatchable_command(wake_command) and wake_command.get("action") == "howie_message":
            log.info("Skipping Vosk fast dispatch for Howie message body=%r; using full STT.", wake_body)
            wake_command = {"action": "none"}
        if is_dispatchable_command(wake_command):
            ...
            dispatcher.dispatch_async(wake_command)
            dispatcher.transcription_async(
        ...
        stt_source = "local"
        followup_text = ""
        followup_wait_seconds = 0.0
        followup_speech_seconds = 0.0
        followup_audio_len = 0
        if not is_dispatchable_command(command):
            followup_audio, followup_wait_seconds, followup_speech_seconds = capture_followup_command(
                mic,
                rolling,
                noise,
            )
            followup_audio_len = len(followup_audio)
            if followup_audio_len:
                followup_text = transcribe(
                    followup_audio,
                    args.whisper_model,
                    args.whisper_no_speech_threshold,
                )
                followup_body, followup_command = command_from_followup_transcript(
                    followup_text,
                    howie_context=howie_wake_context,
                )
                if followup_command.get("action") != "none" and not is_dispatchable_command(followup_command):
                    log.info(
                        "Rejecting incomplete follow-up command body=%r command=%s",
                        followup_body,
                        followup_command,
                    )
                    followup_command = {"action": "none"}
                if is_dispatchable_command(followup_command):
                    log.info(
                        "Follow-up command transcript=%r body=%r command=%s wait=%.0fms speech=%.0fms",
                        followup_text,
                        followup_body,
                        followup_command,
                        followup_wait_seconds * 1000,
                        followup_speech_seconds * 1000,
                    )
                    text = followup_text
                    body = followup_body
                    command = followup_command
                    stt_source = "local_followup"
        force_howie_openai = bool(args.howie_openai_stt and howie_wake_context)
        openai_audio = followup_audio if force_howie_openai and followup_audio_len else command_audio
        openai_mode = "diagnostic" if force_howie_openai else args.openai_stt_mode
        openai_diag = run_openai_stt_if_needed(
            openai_audio,
            openai_mode,
            False if force_howie_openai else is_dispatchable_command(command),
            openai_api_key,
            args.openai_stt_model,
            args.openai_stt_timeout_seconds,
            (
                lambda cloud_text: parse_howie_openai_transcript(
                    cloud_text,
                    wake_text=wake_text,
                    followup_context=bool(followup_audio_len),
                )
            )
        ...
        if force_howie_openai and is_dispatchable_command(openai_diag.command or {}):
            log.info(
                "OpenAI STT refined Howie command transcript=%r body=%r command=%s latency=%sms",
                openai_diag.transcript,
                openai_diag.body,
                openai_diag.command,
                openai_diag.latency_ms,
            ...
            body = openai_diag.body
            command = openai_diag.command or {"action": "none"}
            stt_source = "openai_howie"
        elif args.openai_stt_mode == "fallback" and not is_dispatchable_command(command) and is_dispatchable_command(
            openai_diag.command or {}
        ):
            log.info(
                "OpenAI STT fallback recovered command transcript=%r body=%r command=%s latency=%sms",
                openai_diag.transcript,
            ...
            log.info(
                ...
                openai_diag.body,
                openai_diag.command,
                is_dispatchable_command(openai_diag.command or {}),
                openai_diag.latency_ms,
                openai_diag.error,
            )
        if (
            ...
            fallback_body, fallback_command = howie_fallback_from_transcript(fallback_text, wake_text=wake_text)
            if is_dispatchable_command(fallback_command):
                log.info(
                    "Routing non-command wake transcript to Howie body=%r command=%s",
                    fallback_body,
                    fallback_command,
                )
                text = fallback_text
        ...
        reliability.event(
            ...
            openaiStt={
                ...
                "body": openai_diag.body,
                "command": openai_diag.command,
                "dispatchable": is_dispatchable_command(openai_diag.command or {}),
                "latencyMs": openai_diag.latency_ms,
                "error": openai_diag.error,
            },
            captureMs=round(len(command_audio) / SAMPLE_RATE * 1000),
            preMs=round(min(len(pre_audio), len(command_audio)) / SAMPLE_RATE * 1000),
            postMs=round(post_seconds * 1000),
            followupTranscript=followup_text,
            followupWaitMs=round(followup_wait_seconds * 1000),
            followupSpeechMs=round(followup_speech_seconds * 1000),
            followupCaptureMs=round(followup_audio_len / SAMPLE_RATE * 1000),
            totalMs=round((stt_done - wake_at) * 1000),
        )
        log.info(
            "Command transcript=%r body=%r command=%s capture=%.0fms pre=%.0fms post=%.0fms "
            "followupWait=%.0fms followupSpeech=%.0fms total=%.0fms",
            text,
            body,
            command,
            len(command_audio) / SAMPLE_RATE * 1000,
        ...
        if is_dispatchable_command(command):
            dispatcher.dispatch_async(command)
            dispatcher.transcription_async(final_text, True, wakeScore=1.0, stage="command", command=command)
            reliability.event(
                "dispatch",
                wakeEngine=args.wake_engine,
                source=wake_source,
