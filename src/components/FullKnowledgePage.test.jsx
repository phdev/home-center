...
describe("FullKnowledgePage", () => {
  ...
  it("renders query-type profile maps and infographic modules", () => {
    render(<FullKnowledgePage response={response({
      ...
      visualPlan: {
        moduleStyles: { facts: "compact-fact-rows", middle: "world-map-pin", lower: "island-shape-stats" },
      },
      infographics: [{
        title: "Island scale",
        kind: "comparison",
        description: "Compare Madagascar with nearby land areas.",
        visual: { url: "/home-center/knowledge-assets/madagascar-island-relief.svg", alt: "Teal relief map of Madagascar" },
    ...
    expect(screen.getByAltText("Teal relief map of Madagascar")).toBeTruthy();
  });
  ...
  it("renders Apollo 11 with canonical places, full date, result ornament, and mission glance", () => {
    ...
    expect(container.querySelectorAll(".knowledge-place-callout:not(.knowledge-place-callout-empty)")).toHaveLength(2);
    expect(container.querySelectorAll(".knowledge-us-poi-label")).toHaveLength(0);
    expect(container.querySelector(".knowledge-module-horizontal-mission-timeline")).toBeTruthy();
    expect(container.querySelectorAll(".knowledge-timeline-connector")).toHaveLength(3);
