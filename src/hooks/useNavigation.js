...
function clearForcedPageParams() {
  if (typeof window === "undefined" || !window.history?.replaceState) return;
  const url = new URL(window.location.href);
  if (!url.searchParams.has("page") && !url.searchParams.has("view")) return;
...
export function useNavigation(workerSettings) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (nav.page) {
          if (nav.page === "dashboard") clearForcedPageParams();
          setPage(nav.page);
        }
  ...
  const goTo = useCallback((newPage, newView) => {
    if (newPage) {
      if (newPage === "dashboard") clearForcedPageParams();
      setPage(newPage);
    }
    if (newView) setCalendarView(newView);
  }, []);

  return { page, calendarView, navigationTimestamp: lastTimestamp, goTo };
}
