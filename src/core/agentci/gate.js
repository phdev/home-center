...
function evaluateExpected(snapshot, replay, expected, goldenDiff, hasStoredGolden) {
  ...
  if (Number.isInteger(expected.cardCount)) {
    out.push({
      name: `selected card count remains ${expected.cardCount}`,
      passed: cards.length === expected.cardCount,
    });
  }

  for (const absentCardId of expected.absentCards ?? []) {
    out.push({
      name: `card ${absentCardId} remains absent`,
      passed: !cards.some((item) => item.id === absentCardId),
    });
  }
  ...
  for (const expectedCard of expected.cards ?? []) {
    ...
    if (card && expectedCard.type) {
      out.push({
        name: `card ${expectedCard.id} type remains ${expectedCard.type}`,
        passed: card.type === expectedCard.type,
      });
    }
    if (card && expectedCard.placement) {
      out.push({
        name: `card ${expectedCard.id} placement remains ${expectedCard.placement}`,
        passed: card.placement === expectedCard.placement,
      });
    }
    if (card && Number.isInteger(expectedCard.index)) {
      out.push({
        name: `card ${expectedCard.id} remains at index ${expectedCard.index}`,
        passed: cards[expectedCard.index]?.id === expectedCard.id,
      });
    }
    if (card) {
      out.push({
