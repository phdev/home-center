...
describe("AgentCI offline boundary", () => {
  ...
  it("gate can assert card count and absent cards", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "agentci-"));
    const fixturePath = path.join(dir, "fixture.json");
    const fixtureWithStrictCards = structuredClone(fixture);
    fixtureWithStrictCards.expected = {
      ...fixtureWithStrictCards.expected,
      cardCount: 1,
      absentCards: ["lunch"],
    };
    await writeFile(fixturePath, JSON.stringify(fixtureWithStrictCards, null, 2));
    ...
    expect(result.assertions).toContainEqual({
      name: "selected card count remains 1",
      passed: true,
    });
    expect(result.assertions).toContainEqual({
      name: "card lunch remains absent",
      passed: true,
    });
  });
