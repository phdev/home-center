...
describe("intervention engine", () => {
  ...
  it("folds standalone Peter 8-9 risk into the morning runway card", () => {
    ...
    const derived = computeDerivedState(
      {
        ...
        calendar: {
          events: [
            {
              ...event("focus", at(2026, 4, 23, 8), at(2026, 4, 23, 9)),
              calendarOwner: "peter",
            },
          ],
        },
        checklist: { items: [{ id: "backpacks", label: "Backpacks", condition: "always" }] },
      },
      { now, user: USER },
    );
    const cards = runInterventionEngine(derived, { now });

    expect(derived.peter0800_0900Risk).toBe(true);
    expect(derived.hasMorningOverlap).toBe(false);
    expect(cards.map((card) => card.type)).toEqual(["MorningChecklistCard"]);
  });
