...
DESIGN_OUTPUTS_DAILY = REPO_ROOT / "design_outputs" / "daily"
...
def _without_generated_timestamp(content: str) -> str:
    return "\n".join(
        line for line in content.splitlines() if not line.startswith(GENERATED_PREFIX)
    )


def _has_semantic_change(path: Path, content: str) -> bool:
    if not path.exists():
        return True
    existing = path.read_text(encoding="utf-8")
    return _without_generated_timestamp(existing) != _without_generated_timestamp(content)
...
def write_shared_design_memory_pack() -> list[Path]:
    ...
    for path in (OPENCLAW_PACK_PATH, HOME_CENTER_PACK_PATH):
        ...
        if not _has_semantic_change(path, content):
            continue
