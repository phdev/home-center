...
const pages = [
  ...
  { slug: "full-knowledge-page", nodeId: "J66tW4" },
  { slug: "full-history-page", nodeId: "Tbtje" },
  { slug: "transcription-overlay", nodeId: "DeP7G" },
  { slug: "voice-transcription-overlay", nodeId: "Jf7Tx" },
  { slug: "openclaw-ui-additions", nodeId: "aMgUJ" },
];

const MCP_URL = process.env.PENCIL_MCP_URL;

function saveScreenshotResult(page, msg) {
  let wrote = false;
  if (msg.error) {
...
if (MCP_URL) {
  const initRes = await fetch(MCP_URL, {
    ...
    body: JSON.stringify({
      ...
      id: ++msgId,
      method: "initialize",
      params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "ss", version: "1" } },
    }),
  });
  const sessionId = initRes.headers.get("mcp-session-id");
  if (!initRes.ok || !sessionId) {
    throw new Error(`Failed to initialize Pencil MCP HTTP session: ${initRes.status}`);
  }
  ...
  for (const page of pages) {
    ...
    const res = await fetch(MCP_URL, {
      ...
      body: JSON.stringify({
        ...
        id: ++msgId,
        method: "tools/call",
        params: { name: "get_screenshot", arguments: { filePath: PEN_FILE, nodeId: page.nodeId } },
      }),
    });
    saveScreenshotResult(page, await res.json());
  }
