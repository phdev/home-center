...
a photographic-quality `<stem>-polish.png` via OpenAI image editing
(`gpt-image-2` by default). The structural PNG is fed in as a reference
...
IMAGE_MODEL = os.environ.get("DESIGN_CLAW_IMAGE_MODEL", "gpt-image-2")
IMAGE_QUALITY = os.environ.get("DESIGN_CLAW_IMAGE_QUALITY", "high")
# GPT image models accept 1024x1024 / 1024x1536 / 1536x1024. 1536x1024 is the
...
def generate_polish(payload: dict[str, Any], structural_png: Path, client) -> bytes:
    ...
    # GPT image models return b64_json in response.data[0].b64_json.
