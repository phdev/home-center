...
- **One concept per day.** Rotates through `design_inputs/daily_topics.json`,
  ...
  and asks `gpt-5.5` for a single structured concept.
...
- **Not approved deterministic behavior.** A daily artifact can propose card
  hierarchy, ordering, or suppression, but it cannot become deterministic card
  behavior until Devon runs `/plan-eng-review` and the derived-state contract,
  tests, and docs impact are accepted.
...
| `DESIGN_CLAW_MODEL` | all text LLM calls (via `_design_claw.MODEL`) | optional — defaults to `gpt-5.5`. Set to try a newer model without editing code. |
| `DESIGN_CLAW_IMAGE_MODEL` | polish-pass image generation (`render_polish.py`) | optional — defaults to `gpt-image-2`. Set to swap the image model. |
| `DESIGN_CLAW_IMAGE_QUALITY` | polish-pass image quality (`render_polish.py`) | optional — defaults to `high`. |
...
5. Parse the fenced JSON concept. Save:
   ...
   These outputs are staged first and published atomically: the visible
   daily artifact paths and `.last_daily` pointer are not updated until the
   Markdown/JSON set exists and optional render/polish work has either
...
7. **Polish pass** (runs after structural when `--render` is set and
   `--no-polish` is not):
   - `scripts/render_polish.py` calls OpenAI image editing
     (`gpt-image-2` by default, quality `high`) via `images.edit`, passing the
...
## David Chat + Explicit Memory Feedback via Telegram (listener)

After setup on the Mac Mini, a companion launchd job
(`com.homecenter.design-claw-listener`) polls `getUpdates` every five
minutes for messages from the configured chat.

Ordinary messages receive a short David design/frontend response. David uses
`agents/david_gstack.md`, `claws/david_chat.md`, and the design memory summary
as guidance, but does not update memory unless explicitly asked through the
memory prefixes below.

The listener treats a message as memory feedback only when the text starts with
one of:
...
When it sees explicit memory feedback, it:
...
Messages that start with `/start`, `/help`, `/ack` are ignored. Messages from
other chats are ignored for safety.
...
| `claws/david_chat.md` | Prompt — conversational David design/frontend replies |
...
| `scripts/design_listener.py` | The listener — pulls new DMs, replies as David or merges explicit memory feedback |
