...
## 2026-05-20 — Explicit household memory commands enter live Howie replies

**Context**
Peter asked to wire the household memory adapter into live Howie replies, but
only for the explicit command formats already supported by the adapter.

**Decision**
Add a small Telegram bridge wrapper in `openclaw/household-memory-live.js` and
call it from `openclaw/index.js` before the LLM/worker reply path. If the
adapter parser does not recognize a memory command, the bridge continues with
the existing reply path unchanged. Recognized memory commands reply directly
with the adapter message and are not queued for the non-memory polling path.

**Consequence**
Live Howie can now remember, correct, forget, recall, and source-trace explicit
household memory commands. There is still no passive ingestion, Gmail/calendar
or school-email ingestion, GBrain integration, or dashboard/derived-state
behavior change.

---

## 2026-05-20 — Household memory commands stay conversational-only

**Context**
Howie now has a canonical household memory layer for durable family facts,
routines, preferences, places, school context, and corrections. Peter asked for
an adapter that supports explicit commands like remember, correct, forget,
recall, and source tracing.

**Decision**
Add the command adapter under `src/core/memory/householdMemoryAdapter.js` and
route writes through `applyMemoryUpdate()` and reads through
`queryHouseholdMemory()`. The adapter returns structured conversational results
and refuses to guess when a correction or forget request matches multiple
items.

**Consequence**
Household memory can help Howie answer, explain, and suggest, but it still does
not decide dashboard card visibility, reminders, priority, ordering,
suppression, or derived-state truth. Those remain owned by the deterministic
state/intervention model.

---

## 2026-05-18 — Knowledge bridge uses GPT-5.4-mini before local Gemma

**Context**
Family-facing knowledge cards need factual phrasing and reliable visual-source
decisions. Recent Sun/Moon voice queries showed local Gemma was too inconsistent
for the general knowledge JSON path when the OpenClaw bridge is enabled.

**Decision**
Keep the worker retrieval and visual pipeline, but make the OpenClaw
`/knowledge-json` bridge try OpenAI `gpt-5.4-mini` first when `OPENAI_API_KEY`
is configured. Local Gemma remains a fallback, and Sonnet remains the final
cloud fallback if local completion or JSON parsing fails.

**Consequence**
Production currently reports `knowledgeTextModel: gpt-5.4-mini` with no
knowledge bridge configured. If the Mac bridge is re-enabled later, it will no
longer silently route knowledge answers through local Gemma first.

---

## 2026-05-15 — Knowledge text routes through OpenClaw bridge first

**Context**
The worker knowledge pipeline already owns the product path for family knowledge
queries: classify the question, retrieve source context, choose or generate
visuals, store a dashboard-shaped response. Its text calls were direct OpenAI
calls, while the Gemma/Sonnet router only ran after the worker failed. That made
Gemma/Sonnet a degraded fallback without access to the same retrieved context.

**Decision**
Keep the worker retrieval and visual pipeline as the primary knowledge path, but
route its JSON text-generation calls through the Mac OpenClaw bridge when
`KNOWLEDGE_TEXT_BRIDGE_URL` is configured. Superseded on 2026-05-18: the bridge
now calls OpenAI `gpt-5.4-mini` first, then falls back to local Gemma and
Sonnet. Direct OpenAI remains a worker fallback for bridge outages or
unconfigured bridge deployments.

**Consequence**
Howie remains the family-facing surface and OpenClaw remains the local
orchestrator. Knowledge answers can use local Gemma/Sonnet while preserving the
worker's source-grounded retrieval, visual handling, and dashboard response
contract.

---

## 2026-05-06 — Split dirty work before new builds

**Context**
A broad dirty state accumulated across voice reliability, kiosk wakeword and
autostart updates, docs/design memory, app caption tweaks, Devon index output,
and David daily artifacts. That made review harder because unrelated changes
shared one working tree and generated files were mixed with implementation.

**Decision**
Before building more on a broad dirty tree, split the work into small reviewable
commits by concern. Start with `git status --short`, name the intended commit
buckets, verify each coherent concern with the smallest relevant gate, and
commit it immediately. Keep generated artifacts separate from implementation.
Regenerate `docs/status/devon-project-index.md` only after intentional commits,
so it records a clean known point. For design-to-implementation work, commit
design memory/artifacts first, implementation second, and verification/index
updates last. Include `docs/DESIGN_MEMORY.md` with design-memory/artifact
commits when its body changes, but skip or revert timestamp-only diffs.

**Consequence**
Future work should not proceed on top of unrelated dirty files by default. If a
quick fix appears midstream, pause to commit or stash the current bucket, or
write the fix down and defer it. Devon should call out review risk and propose a
split whenever the working tree spans multiple concerns.

---

## 2026-05-05 — Diagnose voice navigation misses from wake evidence first

**Context**
Peter reported that `Hey Homer, open calendar` was not working. The first
hypothesis was a kiosk/Worker/Pi navigation routing mismatch, but the live Pi
kiosk was already using local navigation. The reliability log showed Vosk heard
the phrase as `hey i'm robyn calendar`: it caught `calendar` but did not match
the wake phrase.

**Decision**
For future voice navigation misses, inspect
`voice-service/logs/voice-reliability.jsonl` and `voice-stderr.log` before
changing routing or deploy configuration. Treat "command word heard but no
action" as possibly a wake recognition failure. Add the observed narrow Vosk
alias `hey i'm robyn ...` to `voice-service/intent.py`, with a regression test
that `hey i'm robyn calendar` dispatches to the calendar page.

**Consequence**
The debugging order is now: mic stream health, Vosk detector text/wake hit,
Whisper command transcript, parser result, Pi dispatch, then kiosk polling.
Do not start by assuming GitHub Pages vs local kiosk routing unless logs show
that dispatch reached `/api/navigate` and the Pi state changed while the
screen did not.

---

## 2026-05-04 — Add voice reliability telemetry for `Hey Homer`

**Context**
Home Center voice commands sometimes failed with no visible action. `Hey Homer,
open calendar` and `Hey Homer, go back` both work when the full path fires, but
some misses had no normal log evidence, making it unclear whether the failure
was mic/audio, Vosk wake detection, Whisper transcription, parser rejection, or
Pi/kiosk dispatch.

**Decision**
Keep production on the existing Mac mini `voice-service` Vosk wake path for now,
but add a structured non-audio reliability log at
`voice-service/logs/voice-reliability.jsonl`. The log records service startup,
loud speech windows, Vosk detector text, wake hits/skips/rejections, Whisper
transcripts, parser results, ignored commands, and dispatches.

Also add a parser fallback for observed navigation phrasing: if Whisper drops
`go` from `go back`, bare `back` and `back please` map to dashboard.

**Consequence**
Future reports of "I said Hey Homer and nothing happened" should start by
checking `voice-reliability.jsonl`, not by guessing thresholds. If repeated
misses show Vosk as the weak point, the next experiment is the existing
confirmed-command speech candidate mode, with passive-TV false positives
measured before making it production.
