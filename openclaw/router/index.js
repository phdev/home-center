...
export async function route(query, context = {}) {
  ...
  const { classification, escalation_target: escalationTarget } = routeInfo;
  ...
  // Try local for local-classified queries.
  if (classification === 'local' && local.available()) {
    try {
      const result = await local.complete(query, context);
      cache.store(query, result.response).catch(() => {});
      return logAndReturn(result, 'local', startTime, query, classification);
    } catch {
      escalatedFrom = 'local';
    }
  ...
  // Groq tier intentionally dormant in the active route. Import/code remains
  // available for explicit compare evals and future re-enablement.
  void groq;

  // Anthropic — sonnet for escalation, opus for hard/safety escalation.
  if (anthropic.available()) {
    const modelKey = escalationTarget === 'opus' ? 'opus' : 'sonnet';
    const result = await anthropic.complete(query, context, modelKey);
    cache.store(query, result.response).catch(() => {});
    return logAndReturn(result, `anthropic-${modelKey}`, startTime, query, classification, {
